...
def load_plm(model_name='multi-qa-mpnet-base-cos-v1'):
    tokenizer = AutoTokenizer.from_pretrained(path)
    model = AutoModel.from_pretrained(path)
    return  tokenizer, model
    #return f"Tokenizer: {str(tokenizer)}, Model: {str(model)}"
tokenizer, model = load_plm(path)
print(type(load_plm(path)))
print("*******")
...
class CGAgent(Agent):
    ...
    def __init__(self, name: str, role: str, rule_role_prompt: str,
        ...
        self.retrival_model =  load_plm(path)#retrival_model
    ...
    def retrival_memory(self, conversations: List[dict]):
        ...
        documents_embedding = outputs.last_hidden_state.mean(dim=1)#self.retrival_model.encode(documents)
        k = min(len(documents), self.T)
        for q in selected_questions + questions:
            q_inputs = tokenizer(q, padding=True, truncation=True, return_tensors='pt')
            with torch.no_grad():
                q_outputs = model(**q_inputs)
            q_embedding = q_outputs.last_hidden_state.mean(dim=1)#model.encode(q)
